...
class LocalHttpEmbeddingModel:
    ...
    def embedding_dimension(self, llm_kwargs: Optional[dict] = None):
        # Mirror OpenAI embed model behavior: read from registry
        from .bridge_all_embed import embed_model_info
        if llm_kwargs is None:
            llm_kwargs = self.llm_kwargs
        if llm_kwargs is None:
            raise RuntimeError("llm_kwargs is not provided!")
        embed_model_name = llm_kwargs.get("embed_model", "local-http-embed")
        dim = embed_model_info[embed_model_name].get("embed_dimension")
        # If not configured, try to auto-detect by probing the endpoint once
        if not dim:
            cached = llm_kwargs.get("_embed_dimension_cache") or self.llm_kwargs.get("_embed_dimension_cache")
            if cached:
                return cached
            try:
                vecs = self.compute_embedding(["dimension-probe"], llm_kwargs=llm_kwargs, batch_mode=True)
                dim = len(vecs[0]) if vecs and isinstance(vecs[0], list) else None
                if dim:
                    # cache in both places to avoid repeated probes
                    self.llm_kwargs["_embed_dimension_cache"] = dim
                    llm_kwargs["_embed_dimension_cache"] = dim
            except Exception:
                pass
        return dim
